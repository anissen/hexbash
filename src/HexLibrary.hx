...
class HexTools {
    ...
    static public function breadthFirstSearch(start :Hex, maxMovement :Int, maxMagnitude :Int, is_walkable :Hex -> Bool) :{ cost_so_far :Map<String, Int>, came_from :Map<String, Hex> } {
        ...
        var fringes = [[start]];
        for (k in 0 ... maxMovement) {
            if (fringes[k].length == 0) break;
            fringes[k + 1] = [];
            for (hex in fringes[k]) {
                for (dir in 0 ... 6) {
                    var neighbor = neighbor(hex, dir);
                    if (cost_so_far.exists(neighbor.key) || !is_walkable(neighbor) || length(neighbor) > maxMagnitude) continue;

                    cost_so_far[neighbor.key] = k + 1;
                    came_from[neighbor.key] = hex;
